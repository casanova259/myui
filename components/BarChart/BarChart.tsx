        ...
        <div className="w-full rounded-2xl border border-white/10 bg-[#0F0F0F]  px-5 pt-5 pb-4 select-none overflow-hidden">
            ...
            <div className="relative h-36 flex items-end gap-1.5">
                ...
                {data.map((d, i) => (
                    <div
                        key={d.month}
                        className="relative flex-1 flex items-end gap-1 h-full cursor-pointer pointer-events-none"
                    ...
                    >
                        ...
                        {/* Desktop bar */}
                        <motion.div
                            className="flex-1 rounded-t-[3px] bg-white hover:brightness-110"
                            initial={{ height: 0 }}
                            animate={{ height: getH(d.desktop) }}
                            transition={{ duration: 0.65, delay: i * 0.09, ease: [0.34, 1.56, 0.64, 1] }}
                            style={{ originY: 1 }}
                            whileHover={{}}
                        />
                        ...
                        {/* Mobile bar */}
                        <motion.div
                            className="flex-1 rounded-t-[3px] bg-white/40 hover:brightness-110"
                            initial={{ height: 0 }}
                            animate={{ height: getH(d.mobile) }}
                            transition={{ duration: 0.65, delay: i * 0.09 + 0.05, ease: [0.34, 1.56, 0.64, 1] }}
                            style={{ originY: 1 }}
                        />
                    </div>
                ))}
            ...
            {/* <div className="flex justify-center gap-5 mt-3">
                {[
                    { label: "Desktop", cls: "bg-white" },
                    { label: "Mobile", cls: "bg-white/40" },
                ].map((item) => (
                    <div key={item.label} className="flex items-center gap-1.5 text-[11px] text-white/50">
                        <div className={`w-2.5 h-2.5 rounded-[2px] ${item.cls}`} />
                        {item.label}
                    </div>
                ))}
            </div> */}
